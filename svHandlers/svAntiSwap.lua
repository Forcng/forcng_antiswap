...
exports.ox_inventory:registerHook('swapItems', function(data)
    ...
    if (action == "take" or action == "give") and blockedItems[itemName] then return false end

    if action == "move" and data.toType == "drop" and blockedItems[itemName] then return false end

    if action == "move" and data.toType == "glovebox" and blockedItems[itemName] then return false end
end, {
    print = false
})
